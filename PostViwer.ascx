...
<ItemTemplate>
    ...
    <td class="content" style="vertical-align:top; padding-left:5px;">
        ...
        <div class="pvmain"><h3><a class="Atitle" href='blog.aspx?bid=<%# Eval("BlogID") %>'><asp:Literal ID="lblArticleTitleText" runat="server"/></a></h3>
            <p><asp:Literal ID="lblText" runat="server"/></p>
        </div></a>
    </td></tr></table></div>
    </td>
</ItemTemplate>
...
<asp:SqlDataSource ID="sdsBlog" runat="server" ConnectionString="<%$ ConnectionStrings:WantGooConnection %>" SelectCommand="SELECT Top(5) BlogId,Good,Blog.Tag,BlogTagClass.BlogTagId,BlogTagClass.TagNoSplit,MemberNo,TopPic,ArticleTitle,Substring(Blog.ArticleText,1,2500) as ArticleText,Preview,OutlineText,PublishTime,IsSell,IsSellShare,PreviewWordCount, ArticleID FROM Blog INNER JOIN BlogTagClass ON Blog.Tag = BlogTagClass.BlogTagId WHERE (IsCopy = 0) AND (Show = 1) AND (IsTopIndex > 0) AND (IsDelete = 0 or IsDelete is NuLL) ORDER BY IsTopIndex DESC,PublishTime DESC"  CacheDuration="600" EnableCaching="True" >
<SelectParameters>
