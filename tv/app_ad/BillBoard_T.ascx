...
<asp:sqldatasource id="sdsStock" runat="server" 
    ...
    selectcommand="SELECT Top (5) Name, StockNo, Deal, Last, [Open], Change, TotalVolume, CASE (Last) WHEN 0 THEN 0 ELSE (Change /Last) END AS percentage FROM Stock WHERE StockNo <> '0000' AND StockNo <> '0081' AND StockNo <> '0080' AND (Market <=1)  Order by percentage DESC" 
...
<asp:SqlDataSource ID="sdsStock3" runat="server" 
    ConnectionString="<%$ ConnectionStrings:twStocksConnectionString %>" 
    selectcommand="SELECT Top(5) Name, StockNo, Deal, Last, [Open], Change, TotalVolume, CASE (Last) WHEN 0 THEN 0 ELSE (Change /Last) END AS percentage FROM Stock WHERE (StockNo <> '0000') AND (StockNo <> '0081') AND (StockNo <> '0080') AND (Last <> '0') AND (Last <> '') AND (Market <=1) ORDER BY percentage" 
    EnableCaching="True" CacheDuration="600">
    <SelectParameters>
        <asp:Parameter DefaultValue="0" Type="Int32" />
    </SelectParameters>
...
<asp:SqlDataSource ID="sdsStock2" runat="server" 
    ConnectionString="<%$ ConnectionStrings:twStocksConnectionString %>" 
    selectcommand="SELECT Top(5) Name, StockNo, Deal, Last, [Open], Change, TotalVolume, CASE (Last) WHEN 0 THEN 0 ELSE (Change /Last) END AS percentage FROM Stock WHERE (StockNo <> '0000') AND (StockNo <> '0081') AND (StockNo <> '0080') AND (Last <> '0') AND (Last <> '') AND (Market <=1) ORDER BY TotalVolume DESC" 
    EnableCaching="True" CacheDuration="600">
    <SelectParameters>
        <asp:Parameter DefaultValue="0" Type="Int32" />
    </SelectParameters>
